...
def get_details():
    ...
    print(choices)
    for stream in list(choices.keys()):
        val.append(stream)
...
def get_vid():
    url = url_var.get()
    req = choice.get()
    path = save_box_var.get()
    dwnld_vid(url, req, path)
...
download = Button(right_overlay, text="Download", bg=dwnld,
                  fg="white", font=("roboto condensed", 20, "bold"), relief=GROOVE, command=get_vid)
